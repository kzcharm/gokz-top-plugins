...
// - Ensure config files exist (AutoExecConfig for main config, manual generation for apikey.cfg)
...
static void CreateApiKeyConfigIfNotExists()
{
    char configPath[PLATFORM_MAX_PATH];
    BuildPath(Path_SM, configPath, sizeof(configPath), "cfg/sourcemod/gokz-top/apikey.cfg");
    ...
    BuildPath(Path_SM, dirPath, sizeof(dirPath), "cfg/sourcemod/gokz-top");
    ...
    if (file != null)
    {
        file.WriteLine("// This file was auto-generated by GOKZTop Core");
        file.WriteLine("// https://github.com/kzcharm/gokz-top-plugins");
        file.WriteLine("");
        file.WriteLine("// GOKZTop API key used by server-side plugins");
        file.WriteLine("// Set in cfg/sourcemod/gokz-top/apikey.cfg");
        file.WriteLine("// ");
        file.WriteLine("// IMPORTANT: Get your API key from gokz.top and paste it below");
        file.WriteLine("// Example: gokz_top_apikey \"your-api-key-here\"");
        file.WriteLine("gokz_top_apikey \"\"");
        file.WriteLine("");
        delete file;
    }
}
...
public void OnPluginStart()
{
    ...
    // Generate the file if it doesn't exist (instead of using AutoExecConfig which can override it)
    CreateApiKeyConfigIfNotExists();

    // Create the ConVar manually (not via AutoExecConfig)
    gCvarApiKey = CreateConVar(
        "gokz_top_apikey",
        "",
    ...
    // Execute both configs explicitly to ensure they apply
